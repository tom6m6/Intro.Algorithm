...
int main() {
    ...
    while (getline(f, line)) {
        ...
        if (name == "香港") hongkong = k;
        if (name == "澳门") aomen = k;
        if (name == "广州市") guangzhou = k;
        iss >> longitude;
        iss.ignore();
        iss>> latitude;
    ...
    for (int i = 0; i < N; i++) {
        ...
        for(int j = 0; j < N; j++) { //更新生成树外的点到树的距离
            if (!st[j] && dist[j] > g[t][j]) {
                if (t == hongkong && j == aomen) continue;
                if (t == aomen && j == hongkong) continue;
                if (t == aomen && j == guangzhou) continue;
                if (t == guangzhou && j == aomen) continue;
                dist[j] = g[t][j];
                pre[j] = t;
            }
        }
